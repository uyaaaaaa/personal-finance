...
依存方向はアーキテクチャ規約の中核であり、検査を伴わなければ文章として残るだけになる。
...
検査の一覧を文書に置かず、スクリプト1つに集約する。一覧は決定論的な情報であり、文書とスクリプトの二重管理は乖離を生むためである（CLAUDE.md「制約とコンテキストの置き場所」）。コンポーネントを追加したときは、その検査をスクリプトへ加える。
